...
const drawerCover = require('../../../../assets/images/bg.png');
const drawerImage = require('../../../../assets/images/avatar.png');
  ...
  renderList(datas) {
    return (
      <List
        dataArray={datas}
        renderRow={data => (
          <ListItem button noBorder onPress={() => NavigationService.navigate(data.route)}>
            <Left>
              <Icon
                active
                name={data.icon}
    ...
    return (
      <Container>
        <Content bounces={false} style={{ flex: 1, top: -1 }} render>
          ...
          <View>{this.renderList(datas2)}</View>
        </Content>
      </Container>
    );
  }
